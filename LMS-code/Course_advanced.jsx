  ...
  return (
    <header className="flex justify-between items-center bg-white px-6 py-3 shadow-sm border-b">
      ...
      <div className="flex items-center gap-4">
        <div className="relative flex items-center border rounded-lg bg-gray-100 px-3 py-2 w-72">
          <FiSearch className="text-gray-500 mr-2" />
          <input
            type="text"
            placeholder="Search"
            className="bg-transparent outline-none w-[250px] text-sm"
          />
        </div>
        <button className="relative p-2 bg-transparent hover:bg-transparent focus:bg-transparent active:bg-transparent">
...
const Sidebar = () => {
  return (
    <aside className="w-64 bg-[#1E1E50] text-white p-5 h-screen">
...
function CreateCourse() {
  ...
  const [sections, setSections] = useState([{ name: '', lectures: [], files: [] }]);
  ...
  const [dropdownOpen, setDropdownOpen] = useState(null); // Track which section's dropdown is open
  ...
  const handleAddInstructor = (event) => {
    ...
    if (instructor && !selectedInstructors.some((inst) => inst.id === instructorId)) {
      setSelectedInstructors([...selectedInstructors, instructor]);
    }
  };

  const handleRemoveInstructor = (id) => {
    setSelectedInstructors(selectedInstructors.filter((inst) => inst.id !== id));
  };
  ...
  const addLecture = (sectionIndex) => {
    ...
    setCurrentLecture({ section: sectionIndex, lecture: newSections[sectionIndex].lectures.length - 1 });
  };
  ...
  const handleFileUpload = (e, sectionIndex, type) => {
    ...
    if (file) {
      const newSections = [...sections];
      newSections[sectionIndex].files.push({ name: file.name, type: type, url: URL.createObjectURL(file) });
      setSections(newSections);
    }
  };
  ...
  const handleSaveAndNext = () => {
    ...
    const allInstructors = [
      { id: "instructor1", name: "John Doe", role: "UI/UX Designer", img: instructor1 },
      { id: "instructor2", name: "Jane Smith", role: "Frontend Developer", img: instructor2 },
        ...
        <div className="p-5">
          <div className="flex flex-wrap mb-5">
            {['basic', 'advance', 'curriculum', 'publish'].map((tab) => (
              <div
                key={tab}
                className={`p-2 px-5 rounded-md mr-2 mb-1 cursor-pointer ${activeTab === tab ? 'bg-[#ffbf00] text-black font-bold' : 'bg-gray-300'}`}
              ...
              <div>
                ...
                <div className="flex justify-between items-center mt-2">
                  ...
                  <div className="flex gap-2">
                    <button className="px-3 py-1 bg-red-100 text-red-600 rounded">Save</button>
                    <button className="px-3 py-1 bg-red-200 text-red-600 rounded">Save & Preview</button>
                  </div>
                </div>
                <form className="mt-3 grid grid-cols-2 gap-3 text-xs">
                  <div className="col-span-2">
                    <label className="block text-gray-700">Title</label>
                    <input type="text" placeholder="Course title" className="w-full border p-1 rounded text-gray-700" />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-gray-700">Subtitle</label>
                    <input type="text" placeholder="Course subtitle" className="w-full border p-1 rounded text-gray-700" />
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="block text-gray-700">Course Sub-category</label>
                    <select className="w-full border p-1 rounded text-gray-700">
                      <option>Select...</option>
                    </select>
                  </div>
                  <div className="col-span-2">
                    ...
                    <input type="text" placeholder="Main course topic" className="w-full border p-1 rounded text-gray-700" />
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="block text-gray-700">Subtitle Language (Optional)</label>
                    <select className="w-full border p-1 rounded text-gray-700">
                      <option>Select...</option>
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex gap-1">
                      <input type="text" placeholder="Duration" className="w-2/3 border p-1 rounded text-gray-700" />
  ...
  onClick={() => navigate('/content-dashboard')} // Update this line
>
  Cancel
</button>
                    <button className="w-full p-2 text-sm rounded-md cursor-pointer mb-2 bg-[#ffbf00] text-black border-none md:w-[120px]"
            ...
            {activeTab === 'advance' && (
              <div>
                ...
                <div className="flex justify-between items-center mt-2">
                  <h2 className="text-base font-semibold">Advance Information</h2>
                  <div className="flex gap-3">
                    <button className="px-3 py-1 bg-red-200 text-red-600 rounded">Save</button>
                    <button className="px-3 py-1 bg-red-200 text-red-600 rounded">Save & Preview</button>
                  </div>
                </div>
                <div className="flex flex-col mb-5 md:flex-row md:justify-between">
                  <div className="w-full flex flex-col mb-3 md:w-[48%]">
                    <h3 className="text-sm font-bold mb-2 text-center">Course Thumbnail</h3>
                    <div className="flex flex-col items-center text-center border-2 border-dashed border-gray-400 p-5 rounded-md bg-gray-50 flex-grow w-full md:flex-row md:items-center md:text-left">
                      ...
                      <div className="flex-1 pl-5">
                        <p className="text-xs text-gray-600 mb-2">
                          ...
                          1200x800 pixels or 12:8 Ratio. Supported formats:
                          .jpg, .jpeg, .png
                        </p>
                        <input
                          type="file"
                          ...
                          id="thumbnail-upload"
                        />
                        <label htmlFor="thumbnail-upload" className="bg-[#ffe0b2] text-gray-800 border-none p-2 px-4 rounded-md cursor-pointer">
                          {thumbnailName ? 'Change Image' : 'Upload Image'}
                        </label>
                        {thumbnailName && <p className="mt-2 text-gray-700">Uploaded: {thumbnailName}</p>}
                        {thumbnailPreview && (
                          <div className="mt-2">
                            <img
                            ...
                            />
                            <button onClick={handleDeleteThumbnail} className="mt-2 px-2 py-1 bg-red-500 text-white rounded">Delete</button>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="w-full flex flex-col mb-3 md:w-[48%]">
                    <h3 className="text-sm font-bold mb-2 text-center">Course Trailer</h3>
                    <div className="flex flex-col items-center text-center border-2 border-dashed border-gray-400 p-5 rounded-md bg-gray-50 flex-grow w-full md:flex-row md:items-center md:text-left">
                      <div className="flex-0 flex justify-center items-center">
                      ...
                      <div className="flex-1 pl-5">
                        ...
                        <input
                          ...
                          id="trailer-upload"
                        />
                        <label htmlFor="trailer-upload" className="bg-[#ffe0b2] text-gray-800 border-none p-2 px-4 rounded-md cursor-pointer">
                          {trailerName ? 'Change Video' : 'Upload Video'}
                        </label>
                        {trailerName && <p className="mt-2 text-gray-700">Uploaded: {trailerName}</p>}
                        {trailerPreview && (
                          <div className="mt-2">
                            <video
                              controls
                            ...
                            <button onClick={handleDeleteTrailer} className="mt-2 px-2 py-1 bg-red-500 text-white rounded">Delete</button>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                <div className="mb-5">
                  <h3 className="text-sm font-bold mb-2">Course Descriptions</h3>
                  <textarea
                ...
                <div className="flex flex-col items-center mt-5 md:flex-row md:justify-between">
                  <button className="w-full p-2 text-sm rounded-md cursor-pointer mb-2 bg-gray-300 text-black border-none md:w-[120px]"
  ...
  <div>
    <div className="flex justify-between items-center p-2 mb-4">
      ...
      <div className="flex gap-2">
        ...
        <button className="px-3 py-1 bg-red-200 text-red-600 rounded text-sm">
          Save & Preview
        </button>
      </div>
    </div>
    ...
    <div className="mb-4">
      ...
      {thumbnailPreview ? (
        <img src={thumbnailPreview} alt="Thumbnail Preview" className="w-32 h-20 object-cover" />
      ) : (
    ...
    {sections.map((section, sectionIndex) => (
      <div key={sectionIndex} className="mb-4 border border-gray-200 rounded-md">
        <div className="p-3">
            ...
            <div className="flex items-center">
              ...
              {editingIndex === sectionIndex ? (
                ...
                <h3 className="text-sm font-medium">
                  {`Section ${sectionIndex + 1}: ${section.name || 'Section Name'}`}
                </h3>
              )}
            </div>
            <div className="flex items-center">
              <button
                ...
                className="px-3 py-1 mt-2 bg-red-100 text-red-600 rounded text-sm"
              >
                +
              </button>
              {dropdownOpen === sectionIndex && (
                <div className="absolute bg-white border border-gray-200 rounded shadow-md p-2 mt-2" ref={dropdownRef}>
                  <div
                    className="p-2 hover:bg-gray-100 cursor-pointer text-sm"
                    onClick={() => addContentOption('Video', sectionIndex)}
                  >
                    Upload Video
                  </div>
                  <div
                    className="p-2 hover:bg-gray-100 cursor-pointer text-sm"
                    onClick={() => addContentOption('Attach File', sectionIndex)}
                  >
                    Attach File
                  </div>
                </div>
          ...
          <div className="mt-2">
            {section.lectures.map((lecture, lectureIndex) => (
              <div key={lectureIndex} className="flex items-center justify-between border-b border-gray-200 py-2">
                ...
                <div className="flex items-center">
                  <button
                    ...
                    onClick={() => {
                      ...
                      setEditingLecture({ section: sectionIndex, lecture: lectureIndex });
                    }}
                  >
                    <FaEdit className="text-xs" />
                  </button>
            ...
            {section.files.map((file, fileIndex) => (
              <div key={fileIndex} className="flex items-center justify-between border-b border-gray-200 py-2">
                <span className="text-sm">{file.name}</span>
                <a href={file.url} target="_blank" rel="noopener noreferrer" className="text-blue-500 text-xs">View</a>
              </div>
            ))}
          </div>
        </div>
    ...
    <button
      onClick={addSection}
      className="px-4 py-2 bg-red-100 text-red-600 rounded text-sm"
    >
      Add Section
    </button>
    ...
    <div className="mb-6">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        {selectedInstructors.map((inst) => (
          <div key={inst.id} className="flex items-center bg-gray-100 p-2 rounded-lg w-full md:w-64">
            <img
              src={inst.img}
              alt={inst.name}
